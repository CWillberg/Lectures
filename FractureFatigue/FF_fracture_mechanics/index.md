...
theme: h2
paginate: true
header: "Fracture & Fatigue — Chapter 4: Linear Fracture Mechanics"
math: katex
---
...
# 4.2 The Crack Tip Field

---

## 4.2.1 Two-Dimensional Crack Tip Fields — Mode III
...
$$\sin 2\lambda\pi = 0 \quad\Rightarrow\quad \lambda = \frac{n}{2}, \quad n = 1,2,3,\ldots \tag{4.3}$$

The general solution is a superposition of eigenfunctions:

$$\Omega = A_1 z^{1/2} + A_2 z + A_3 z^{3/2} + \ldots \tag{4.4}$$

> **Fig. 4.3** — Coordinate system at the crack tip: radius $r$, angle $\varphi$

---

## 4.2.1 Mode III Crack Tip Field — Dominant Singular Term

As $r \to 0$, the dominant term ($\lambda = 1/2$) gives the **singular crack tip field**:

$$\begin{pmatrix} \tau_{xz} \\ \tau_{yz} \end{pmatrix} = \frac{K_{III}}{\sqrt{2\pi r}} \begin{pmatrix} -\sin(\varphi/2) \\ \cos(\varphi/2) \end{pmatrix}, \qquad w = \frac{2K_{III}}{G}\sqrt{\frac{r}{2\pi}}\sin(\varphi/2) \tag{4.6}$$
...
$$v^\pm = \pm\frac{K_I}{2G}\sqrt{\frac{r}{2\pi}}(\kappa+1) \tag{4.21}$$

This gives a **parabolic crack opening profile** (Fig. 4.6a).

Stresses in polar coordinates:

$$\begin{pmatrix}\sigma_r\\\sigma_\varphi\\\tau_{r\varphi}\end{pmatrix} = \frac{K_I}{4\sqrt{2\pi r}} \begin{pmatrix}5\cos(\varphi/2)-\cos(3\varphi/2)\\3\cos(\varphi/2)+\cos(3\varphi/2)\\\sin(\varphi/2)+\sin(3\varphi/2)\end{pmatrix} \tag{4.22}$$

> **Fig. 4.6** — Mode I crack tip field: (a) crack opening and $\sigma_y$ distribution, (b) angular dependence of stress components

---

## 4.2.2 Principal Stresses and Maximum Shear in Mode I

Principal stresses and directions:

$$\begin{pmatrix}\sigma_1\\\sigma_2\end{pmatrix} = \frac{K_I}{\sqrt{2\pi r}}\cos\frac{\varphi}{2}\begin{pmatrix}1+\sin(\varphi/2)\\1-\sin(\varphi/2)\end{pmatrix}, \quad \alpha = \pm\frac{\pi}{4}+\frac{3}{4}\varphi \tag{4.23}$$

Third principal stress:

$$\sigma_3 = \frac{2\nu K_I}{\sqrt{2\pi r}}\cos\frac{\varphi}{2} \text{ (plane strain)}, \qquad \sigma_3 = 0 \text{ (plane stress)} \tag{4.24}$$
...
$$\sigma_{ij} = \frac{1}{\sqrt{2\pi r}}\left[K_I\tilde{\sigma}^I_{ij}(\varphi) + K_{II}\tilde{\sigma}^{II}_{ij}(\varphi) + K_{III}\tilde{\sigma}^{III}_{ij}(\varphi)\right] \tag{4.26}$$
...
> **Fig. 4.7** — 3D crack tip field: (a) penny-shaped and semi-elliptical surface cracks, (b) local coordinate system at crack front
...
$$K_{II} = K_{IIc} \quad \text{(Mode II)}, \qquad K_{III} = K_{IIIc} \quad \text{(Mode III)} \tag{4.28}$$

General mixed-mode criterion: $f(K_I, K_{II}, K_{III}) = 0$ &nbsp; (4.29) — see Section 4.9.

> **Fig. 4.8** — K-concept: $K_I$-dominated field, plastic zone $r_p$, process zone $\rho$
...
## 4.4.1 Example: Crack Under Remote Tension
...
Result for stresses along the x-axis (Fig. 4.9b):
...
$$\boxed{K_I = \sigma\sqrt{\pi a}} \tag{4.34}$$

> **Fig. 4.9** — Crack under tension: (a) superposition, (b) stress along x-axis, (c) crack opening

---

## 4.4.1 Further Examples

**Point forces on crack faces** (Fig. 4.10a,b):

$$K_I^\pm = \frac{P}{\sqrt{\pi a}}\sqrt{\frac{a\pm b}{a\mp b}} \tag{4.36}$$

**Shear loading** (Mode II, Fig. 4.10e):

$$K_{II} = \tau\sqrt{\pi a} \tag{4.41}$$

**Collinear crack array** (period $2b$, Fig. 4.11a):

$$K_I = \sigma\sqrt{\pi a}\sqrt{\frac{2b}{\pi a}\tan\frac{\pi a}{2b}} \tag{4.43}$$

As crack tips approach each other ($a\to b$), $K_I$ grows strongly due to **crack interaction**.

> **Fig. 4.10** — Crack under various loading conditions; **Fig. 4.11** — (a) collinear crack array, (b) strip with internal crack

---

## 4.4.1 K-Factor Table (Selected Cases)
...
$$\sigma_y(x,0) = -\frac{2G}{\pi(\kappa+1)}\int_{-a}^{+a}\frac{\mu(t)\,dt}{x-t} \tag{4.47}$$

where $\mu(t) = db_y/dt$ is the **dislocation density**.

For the crack under pressure $\sigma$ (Fig. 4.13), this becomes a singular integral equation with solution:

$$\mu(x) = \frac{\sigma(\kappa+1)}{2G}\frac{x}{\sqrt{a^2-x^2}} \tag{4.49}$$

The SIF follows directly from the dislocation density:

$$K_I = \lim_{x\to a}\frac{2G}{\kappa+1}\sqrt{2\pi}\sqrt{a-x}\,\mu(x) \tag{4.51}$$

This yields the known result $K_I = \sigma\sqrt{\pi a}$.

> **Fig. 4.12** — Displacement jump from edge dislocation; **Fig. 4.13** — Crack as dislocation distribution; **Fig. 4.14** — 3D curved crack surface $\Gamma$

---

## 4.4.2 General 3D Integral Equation

For a 3D crack surface $\Gamma$ under general loading (Fig. 4.14), the dislocation density tensor is:

$$\mu_{ijk} = \Delta u_{i,j}n_k - \Delta u_{i,k}n_j \tag{4.52}$$

The boundary integral equation:

$$t^p(x) = -n_q(x)C_{pqkl}\int_\Gamma \Sigma_{ijl}(x-y)\mu_{ijk}(y)\,dA(y) \tag{4.53}$$

This formulation is the basis for the **Boundary Element Method (BEM)** — very efficient for crack problems.

The kernel $\Sigma_{ijl}$ has a $r^{-1}$ singularity (same type as in 2D), and SIFs can be extracted from $\mu_{ijk}$ as in the 2D case.

---

## 4.4.3 Weight Function Method
...
$$K_I = -\frac{8G}{\kappa+1}\frac{1}{K_I^r}\int_0^a \sigma_y\frac{\partial v^r}{\partial a}\,dx \tag{4.57}$$
...
$$K_I = \frac{2}{\pi}\sigma_0\sqrt{\pi a} \tag{4.58}$$

**Petroski–Achenbach approximation:** when the reference displacement $v^r$ is unknown, use a two-term ansatz anchored to the near-field solution to obtain approximate weight functions — Fig. 4.16.

> **Fig. 4.15** — Application of Betti's theorem; **Fig. 4.16** — Examples for weight function method

---

## 4.4.4 Crack Interaction (Kachanov Method)
...
$$\Lambda_{12} = \bar{f}_{12} = \frac{1}{c-b}\int_b^c f_{12}(x)\,dx = \frac{\sqrt{c^2-a^2}-\sqrt{b^2-a^2}}{c-b}-1 \tag{4.64}$$

**Kachanov self-consistency equations** for $n$ cracks under Mode I:

$$(\delta_{ji}-\Lambda_{ji})\bar{p}_j = p_i^\infty, \quad i=1,\ldots,n \tag{4.67}$$

**Far-field approximation** ($d\gg a$, collinear cracks):

$$K_I \approx K_I^0\left(1+\frac{1}{2}\left(\frac{a}{d}\right)^2\right) \tag{4.69}$$
...
## 4.4.5 Stress Intensity Factors and Notch Factors

**Elliptical hole** (semi-axes $a$, $b$) under tension $\sigma$ (Fig. 4.21):

Maximum stress at the tip:

$$\sigma_\text{max} = \sigma\left(1+2\sqrt{\frac{a}{\rho}}\right) \approx 2\sigma\sqrt{\frac{a}{\rho}} \quad (\rho\ll a) \tag{4.71/72}$$

where $\rho = b^2/a$ is the tip radius. As $\rho\to 0$: the ellipse degenerates to a crack, $\sigma_\text{max}\to\infty$.

**Parabolic notch** (Fig. 4.22a) — Mode I stress field near tip:

$$\sigma_\text{max} = \frac{\sqrt{2K_I}}{\sqrt{\pi\rho}} \tag{4.76}$$

This relates the **SIF of the associated crack** to the **maximum notch stress** — universally valid for deep notches.

Inverse: $K_I = \lim_{\rho\to 0}\frac{1}{2}\sqrt{\pi\rho}\,\sigma_\text{max}$ &nbsp; (4.77)

> **Fig. 4.21** — Elliptical hole under tension; **Fig. 4.22** — (a) parabolic notch, (b) V-notch

---

## 4.4.6 Numerical Determination of K-Factors

**Finite Element Method (FEM):** entire body discretized into elements (Fig. 4.23a)

**Boundary Element Method (BEM):** only the boundary (including cracks) is discretized (Fig. 4.23b) — particularly efficient for crack problems

**K-factor extraction from displacements** (crack face method):

$$\begin{pmatrix}K_I\\K_{II}\\K_{III}\end{pmatrix} = \lim_{r\to 0}\sqrt{\frac{2\pi}{r}} \begin{pmatrix}v(r,\pi)\cdot 2G/(\kappa+1)\\u(r,\pi)\cdot 2G/(\kappa+1)\\w(r,\pi)\cdot G/2\end{pmatrix} \tag{4.80}$$

**K-factor extraction from stresses** (ahead of crack tip):

$$\begin{pmatrix}K_I\\K_{II}\\K_{III}\end{pmatrix} = \lim_{r\to 0}\sqrt{2\pi r} \begin{pmatrix}\sigma_y(r,0)\\\tau_{xy}(r,0)\\\tau_{yz}(r,0)\end{pmatrix} \tag{4.81}$$
...
$$a,\; W-a,\; B \;\geq\; 2.5\left(\frac{K_{Ic}}{\sigma_F}\right)^2 \tag{4.82}$$

where $\sigma_F$ = yield stress ($R_e$). This guarantees the $K_I$-dominated field is valid.

> **Fig. 4.25** — Specimen geometries: (a) CT specimen, (b) 3PB specimen

---

## 4.5 Factors Influencing $K_{Ic}$
...
> **Fig. 4.26** — (a) Effect of specimen thickness, (b) effect of temperature
...
$$\Delta\Pi = \Delta W_\sigma \leq 0 \tag{4.83}$$
...
$$\Delta\Pi = -\frac{\sigma^2 a^2\pi(1+\kappa)}{8G} \tag{4.85}$$

For fixed displacement:

$$\Delta\Pi = -\frac{\sigma^2 a^2\pi(1+\kappa)}{8G} \tag{4.86}$$
...
$$\mathcal{G} = -\frac{d\Pi}{dA} \qquad \text{(3D)}, \qquad \mathcal{G} = -\frac{d\Pi}{da} \qquad \text{(2D per unit thickness)} \tag{4.87}$$
...
$$\mathcal{G} = \frac{\kappa+1}{8G}K_I^2 = \begin{cases}K_I^2/E & \text{plane stress}\\ (1-\nu^2)K_I^2/E & \text{plane strain}\end{cases} \tag{4.89}$$

For all three modes combined:

$$\mathcal{G} = \frac{1}{E^\ast}(K_I^2 + K_{II}^2) + \frac{1}{2G}K_{III}^2 \tag{4.91}$$
...
$$\mathcal{G} = \frac{F^2}{2B}\frac{dC}{da} \tag{4.95}$$

This result is **independent of loading type** (dead load, spring, or fixed displacement).

For pure Mode I:

$$K_I^2 = \frac{F^2 E^\ast}{2B}\frac{dC}{da} \tag{4.96}$$
...
$$C = \frac{8a^3}{EBh^3}, \qquad K_I = \frac{2\sqrt{3}\,Fa}{Bh^{3/2}} \tag{4.97}$$
...
$$\sigma_c = \sqrt{\frac{16G\gamma}{\pi(1+\kappa)a}}, \qquad a_c = \frac{16G\gamma}{\pi(1+\kappa)\sigma^2} \tag{4.106}$$
...
$$h_c = \frac{2\sqrt{5(1-\nu)}}{3(1-\nu^2)}\frac{G_c E}{\sigma_0^2} \tag{4.111}$$

**Peel test** (Fig. 4.36) — film peeled at angle $\varphi$, elastic deformation included:

$$F = EA\left[\sqrt{\frac{2\gamma}{Eh}+(1-\cos\varphi)^2} - (1-\cos\varphi)\right] \tag{4.113}$$
...
$$\mathcal{G} \approx -\frac{\Pi(a+\Delta a)-\Pi(a)}{\Delta a} \tag{4.115}$$
...
$$J_k = \int_{\partial V}b_{kj}n_j\,dA = \int_{\partial V}(U\delta_{jk}-\sigma_{ij}u_{i,k})n_j\,dA \tag{4.116}$$
...
- $L_k$ (generalized moment / rotational) &nbsp; (4.120)
- $M$ (self-similar defect growth, e.g. void expansion) &nbsp; (4.120)
...
$$J = J_1 = \int_C\left(U\,dy - t_i u_{i,x}\,dc\right) \tag{4.128}$$

**Energetic interpretation:** $J$ = energy release rate for crack advance:

$$J = \mathcal{G} = -\frac{d\Pi}{da} \tag{4.129}$$
...
$$J = \frac{1}{E^\ast}(K_I^2+K_{II}^2) + \frac{1}{2G}K_{III}^2 \tag{4.131}$$
...
$$J = \lim_{\rho\to 0}\int_C(U\,dy - t_i u_{i,x}\,dc) \tag{4.133}$$

The linear elastic relation (4.131) still holds.

**y-component of generalized force** on the crack tip:

$$J_2 = -\frac{1}{E^\ast}K_I K_{II} \tag{4.134}$$
...
$$2r_p = \begin{cases}\dfrac{1}{3\pi}\left(\dfrac{K_I}{\sigma_F}\right)^2 & \text{plane strain (EVZ)}\\ \dfrac{1}{\pi}\left(\dfrac{K_I}{\sigma_F}\right)^2 & \text{plane stress (ESZ)}\end{cases} \tag{4.135}$$
...
$$\mathcal{G}(F,a) = R(\Delta a) \tag{4.142}$$

**Stability condition** (at fixed load):

$$\frac{\partial\mathcal{G}}{\partial a}\bigg|_F < \frac{dR}{da} \quad \Rightarrow\quad \text{stable crack growth} \tag{4.143}$$

**Instability (onset of unstable fracture):**

$$\frac{\partial\mathcal{G}}{\partial a}\bigg|_F = \frac{dR}{da} \tag{4.144}$$
...
$$\frac{d\mathcal{G}}{da} = \frac{F^2}{2}\left[C'' - \frac{2C'^2}{C+C_F}\right] \tag{4.146}$$

Two special cases:

$$\frac{d\mathcal{G}}{da} = \frac{F^2}{2}\begin{cases}C'' - 2C'^2/C & C_F=0 \text{ (fixed displacement)}\\ C'' & C_F\to\infty \text{ (dead load)}\end{cases} \tag{4.147}$$

**Dead load** always reaches instability **earlier** than fixed displacement loading.

**DCB specimen** ($C = 8a^3/EBh^3$):

$$\frac{d\mathcal{G}}{da} = \begin{cases}-48F^2a/EBh^3 & \text{fixed displacement → always stable}\\ +24F^2a/EBh^3 & \text{dead load → unstable}\end{cases} \tag{4.148}$$
...
$$\mathcal{G} = G_c \quad\Rightarrow\quad K_I^2 + K_{II}^2 = K_{Ic}^2 \tag{4.151}$$
...
$$K_I\sin\varphi_0 + K_{II}(3\cos\varphi_0-1) = 0 \tag{4.152a}$$
$$K_I\left(3\cos\frac{\varphi_0}{2}+\cos\frac{3\varphi_0}{2}\right) - K_{II}\left(3\sin\frac{\varphi_0}{2}+3\sin\frac{3\varphi_0}{2}\right) = 4K_{Ic} \tag{4.152b}$$
...
$$U = \frac{S}{r}, \qquad S = \frac{1}{16\pi G}[a_{11}K_I^2 + 2a_{12}K_IK_{II} + a_{22}K_{II}^2] \tag{4.153/154}$$
...
$$k_{II} \approx C_{21}K_I + C_{22}K_{II} + D_2\sigma_T\sqrt{\pi\varepsilon} \tag{4.157}$$

**Small Mode II limit** (all criteria agree):

$$\varphi_0 \approx -2\frac{K_{II}}{K_I} \tag{4.159}$$
...
|-----------|--------------------------|-------------------|
| Energetic (4.151) | tangential (0°) | 1.0 |
| Max. hoop stress (4.152) | −70.6° | 0.866 |
| S-criterion, $\nu=1/3$ (4.155) | −83.6° | 0.905 |
| Kink model (4.156) | ≈ −70° (numerical) | ≈ 0.87 |
...
$$K_I = \sigma\sqrt{\pi a}\cos^2\gamma, \qquad K_{II} = \sigma\sqrt{\pi a}\sin\gamma\cos\gamma \tag{4.160}$$

Applying the maximum hoop stress criterion (4.152):
...
$$\frac{1}{\Delta a}\int_0^{\Delta a}f(\sigma_{ij})\,dx = \sigma_c \qquad \text{(averaged stress criterion)} \tag{4.164a}$$

$$\mathcal{G}(\Delta a) = -\frac{\Delta\Pi}{\Delta a} = G_c \qquad \text{(incremental energy criterion)} \tag{4.164b}$$
...
$$l = \frac{1}{\pi}\left(\frac{K_{Ic}}{\sigma_c}\right)^2 \tag{4.165}$$
...
$$\sigma\sqrt{1+2\frac{a}{\Delta a}} = \sigma_c, \qquad \pi\sigma^2\left(a+\frac{\Delta a}{2}\right) = K_{Ic}^2 \tag{4.166}$$

Solution: $\Delta a = 2l$, and the failure stress:

$$\sigma_f = \frac{K_{Ic}}{\sqrt{\pi a}}\cdot\frac{1}{\sqrt{1+\Delta a/a}} = \sigma_c\sqrt{\frac{1}{1+a/l}} \tag{4.167}$$
...
$$\boxed{\frac{da}{dN} = C\,(\Delta K)^m} \tag{4.168}$$
...
$$\frac{da}{dN} = \frac{C\,(\Delta K)^m}{(1-R)K_{Ic}-\Delta K}, \qquad R = K_\text{min}/K_\text{max} \tag{4.169}$$
...
$$N(a) = \frac{1}{C(\Delta\sigma)^m}\int_{a_i}^{a}\frac{da'}{\left[\sqrt{\pi a'}F(a')\right]^m} \tag{4.170}$$
...
$$\lambda = \frac{1}{2}+i\varepsilon \tag{4.174}$$

where the **bimaterial constant** is:

$$\varepsilon = \frac{1}{2\pi}\ln\frac{\mu_2\kappa_1+\mu_1}{\mu_1\kappa_2+\mu_2} \tag{4.173}$$

Crack tip stresses and displacements show:

$$\sigma_{ij}\sim r^{-1/2}\cos(\varepsilon\ln r), \qquad u_i\sim r^{1/2}\cos(\varepsilon\ln r) \tag{4.175}$$
...
$$\mathcal{G}(\psi) = G_c^{(i)}(\psi), \qquad \tan\psi = K_{II}/K_I \tag{4.187}$$
...
$$\mathcal{G} = f\frac{(1-\nu_1^2)\sigma^2 h_1}{2E_1} \tag{4.190}$$
...
$$\sigma_y = \sigma\frac{x}{\sqrt{x^2-a^2}} \tag{4.193a}$$

This is **identical to the isotropic result** — the SIF is:

$$K_I = \sigma\sqrt{\pi a} \tag{4.194}$$

Same for pure shear: $K_{II} = \tau\sqrt{\pi a}$ &nbsp; (4.195).
...
Mode I and Mode II near-field (Sih et al., 1965) — Eqs. (4.196), (4.197):
...
$$\mathcal{G} = K_I^2\sqrt{h_{11}h_{22}} + K_{II}^2\sqrt{h_{11}}\left[\frac{1}{2}\sqrt{\frac{h_{22}}{h_{11}}}+\frac{2h_{12}+h_{66}}{2h_{11}}\right]^{1/2} \tag{4.198}$$
...
| Topic | Key result |
|-------|-----------|
| Fracture toughness | $K_I = K_{Ic}$, material property, size-dependent measurement |
| Plastic zone (plane strain) | $2r_p = \frac{1}{3\pi}(K_I/\sigma_F)^2$ |
| Plastic zone (plane stress) | $2r_p = \frac{1}{\pi}(K_I/\sigma_F)^2$ |
| Stable crack growth | $\mathcal{G} = R(\Delta a)$, stable if $\partial\mathcal{G}/\partial a < dR/da$ |
| Paris law | $da/dN = C(\Delta K)^m$, $m \approx 2\ldots 4$ for metals |
| Mixed mode kink angle | $\varphi_0 \approx -2K_{II}/K_I$ for small mode II |
| Leguillon criterion | Simultaneous stress ($\sigma_c$) and energy ($G_c$) conditions; size effect captured |
| Interface crack | Oscillating singularity; $\mathcal{G}$ from $K_1^2+K_2^2$; path-dependent mode mix |
| Orthotropy | Same $K_I = \sigma\sqrt{\pi a}$ along symmetry axis; $G_c$ direction-dependent |
